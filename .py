        ...
        try:
           ...
           # try:
              ...
              #      proxies = file.readlines()
               ...
               # proxies = [proxy.strip() for proxy in proxies if proxy.strip() != socks5_proxy]

               # with open('local_proxies.txt', 'w') as file:
               #     file.write('\n'.join(proxies) + '\n')

              #  logger.info(f"Removed faulty proxy {socks5_proxy} from local_proxies.txt")

          #  except Exception as file_error:
            #    logger.error(f"Error while updating local_proxies.txt: {file_error}")
